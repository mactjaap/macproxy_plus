...
def strip_to_html2(html: str) -> str:
    ...
    for h2 in soup.find_all('h2'):
        ...
        if a:
            ...
            h2.replace_with(f"<br>{text}:")
            logger.debug(
                "Flattened Important Information header to %r",
                f"{text}:"
            )

    # ── REMOVE any <h3> tags (and their links) in Important Information blocks ───────────────
...
#    # ── FLATTEN all <time> tags with data-date-string & data-time-string ───────
#    for t in soup.find_all('time', attrs={'data-date-string': True, 'data-time-string': True}):
#        date = t['data-date-string']
#        time = t['data-time-string']
#        # replace the entire <time>…</time> with plain text
#        t.replace_with(f"{date} at {time}")
#        logger.debug("Flattened <time> tag → %r", f"{date} at {time}")

# TIME ON PAGES

        # ── ENHANCED: FLATTEN all <time> tags with relative “ago” + TZ ────────
        import datetime as _dt
        for t in soup.find_all('time', attrs={'datetime': True, 'data-date-string': True, 'data-time-string': True}):
            # parse the ISO datetime (e.g. "2025-06-16T17:09:54+0100")
            dt_str = t['datetime']
            dt_obj = _dt.datetime.strptime(dt_str, "%Y-%m-%dT%H:%M:%S%z")

            # compute “now” in the same tz, and the delta
            now   = _dt.datetime.now(dt_obj.tzinfo)
            delta = now - dt_obj
            ...
            # pull the forum-rendered date & time
            date = t['data-date-string']   # e.g. "Jun 16, 2025"
            time = t['data-time-string']   # e.g. "5:09 PM"
            # get the zone abbreviation (falls back to +HHMM if no name)
            zone = dt_obj.strftime("%Z") or dt_obj.strftime("%z")

            replacement = f"{date} at {time} {zone} ({ago})"
            t.replace_with(replacement)
            logger.debug("Flattened <time> → %r", replacement)
...
# extensions/68kmlaorg/68kmlaorg.py
    ...
    # ── ADD "What’s New" POST-style button after <h1>68kMLA</h1> ──────────────
#    for h1 in soup.find_all('h1', string=lambda t: t and t.strip() == "68kMLA"):
#        # build the form fragment
#        fragment = BeautifulSoup(
#            '<form action="/bb/index.php?whats-new/" method="post">'
#            '<input type="submit" value="What’s new"/>'
#            '</form><br>',
#            'html.parser'
#        )
#        form = fragment.find('form')
#
#        # insert it right after the <h1>
#        h1.insert_after(form)
#        logger.debug("Inserted What’s new button after <h1>68kMLA</h1>")


    # ── ADD inline “What’s New” link after <h1>68kMLA</h1> ────────────────
    for h1 in soup.find_all('h1', string=lambda t: t and t.strip() == "68kMLA"):
        a = soup.new_tag('a', href="/bb/index.php?whats-new/")
        a.string = "What’s new"
        h1.insert_after(a)
...
def handle_request(req):
    ...
    # ─── Handle “What’s new” POST by proxying it as a GET ────────────────────
    if req.method == 'POST' and 'whats-new/' in full:
        url = f"https://{DOMAIN}/bb/index.php?whats-new/"
        logger.debug("Treating What’s new POST as GET → %s", url)
        r = SESSION.get(
            url,
            headers={'User-Agent': request.headers.get('User-Agent', '')}
        )
        ...
        inner = strip_to_html2(r.text)
        title = (orig_soup.title.string if orig_soup.title else "What’s new")
        return wrap_html2(inner, title, debug, user_id), 200
